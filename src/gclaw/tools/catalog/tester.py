...
async def test_tool(record: ToolRecord) -> dict:
    """Dispatch on kind; return a uniform probe-result dict.

    Never raises — every exception is caught and reported via the
    ``error`` field.
    """
    ...
    try:
        ...
        if record.kind == ToolKind.MCP:
            return _not_yet(start, "MCP probe will be wired in Phase 4")
