...
name: adk-patterns
description: >
  Authoritative reference for the Google ADK (Agent Development Kit)
  Python API. Use when writing or modifying ADK agent code: agent
  definitions, tool functions, callbacks, sub-agent orchestration,
  state management, sessions. Pair with `gclaw-conventions` (which
  covers the gclaw-specific manager / specialist / board / soul
  patterns built on top of ADK).
metadata:
  author: Google (vendored from google/agents-cli, Apache-2.0)
  upstream: https://github.com/google/agents-cli/tree/main/skills/google-agents-cli-adk-code
  license: Apache-2.0
  version: 0.1.1
  vendored_at: 2026-04-22
---

# ADK Cheatsheet

> **In gclaw**: this is reference material for the `agent-architect`
> agent (and any other code that touches ADK). gclaw doesn't use the
> `agents-cli` binary at runtime — agents live in-process via the
> gclaw factory. See `gclaw-conventions` for the wrapping patterns.
