...
| 12 | Cloud Run deployment | `adk deploy cloud_run --session_service_uri=agentengine://... --memory_service_uri=agentengine://... .` — one command, ADK generates the Dockerfile and service wiring. | Custom `Dockerfile` + `cloudbuild.yaml` targeting `apex-internal-apps`. FastAPI app, not the ADK-generated web UI. | **DIVERGENT (intentional)** — GClaw has FastAPI routes beyond what ADK's auto-generated app provides (board CRUD, cron routes, heartbeat, auth middleware, routing admin). `adk deploy cloud_run` wouldn't know about any of these. |
